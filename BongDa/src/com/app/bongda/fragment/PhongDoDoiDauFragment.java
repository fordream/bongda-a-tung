...
public class PhongDoDoiDauFragment extends BaseFragment {
	...
	public void onInitData() {
			...
			public void onSuccess(String response) {
				...
				if (!string_temp.equalsIgnoreCase("")) {
					try {
						...
						ImageLoaderUtils.getInstance(null).DisplayImage(sLogoDoiNha, (ImageView) view.findViewById(R.id.logo_doinha));
						ImageLoaderUtils.getInstance(null).DisplayImage(sLogoDoiKhach, (ImageView) view.findViewById(R.id.logo_doikhach));
		...
		callbackAPI = new ICallbackAPI() {
				...
				if (!string_temp.equalsIgnoreCase("")) {
					try {
						...
						for (int i = 0; i < jsonarray.length(); i++) {
							...
							// sHeSo));
							phongdodoidau_bangephang_listitem.addView(new BangXepHangItemView(view.getContext(), phongdo));
						}
			...
			@Override
			public void onError(String message) {
				CommonAndroid.showDialog(view.getContext(), "err:" + message, null);
				Log.e("ERR", message);
			}
		};
		...
		new APICaller(getActivity()).callApi("", true, callbackAPI_tuongthuat, param2);
	}
	...
	private void PhongDoChiTiet() {
		...
		String param2 = (ByUtils.wsFootBall_Phong_Do_ChiTiet).replace("magiai", magiai);
		param2 = param2.replace("madoinha", madoinha);
		param2 = param2.replace("madoikhach", madoikhach);
		new APICaller(getActivity()).callApi("", true, callbackAPI_Chitiet, param2);
	}
}
