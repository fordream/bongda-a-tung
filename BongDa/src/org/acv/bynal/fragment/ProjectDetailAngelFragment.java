...
public class ProjectDetailAngelFragment extends BaseFragment {
	Map<String, String> sendData = new HashMap<String, String>();
	ICallbackAPI callbackAPI;
	GridView pDetailAngelview;
	public ProjectDetailAngelFragment() {
	}
...
//						CommonAndroid.showDialog(getActivity(), "data:" + jsonObject.toString() , null);	
		...
		new APICaller(getActivity()).callApi("/project/listAngel", true,
				callbackAPI, sendData);
		
		pDetailAngelview = (GridView) view.findViewById(R.id.gridview_angel);
		baseAdapter = new BaseAdapter(getActivity(), new ArrayList<Object>()) {
			@Override
			public BaseView getView(Context context, Object data) {
				final ProjectDetailAngelItemView projectDetailAngelItemView = new ProjectDetailAngelItemView(
							context);
